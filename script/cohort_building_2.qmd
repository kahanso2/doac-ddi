...
days_between_fills <- all_drug_index %>%
  ...
  mutate(
    ...
    days_between_fills = ifelse(days_between_fills < 0, 0, days_between_fills)
  ) %>%
...
all_drug_index_continuous <- oac_cohort |> 
  ...
  mutate(oac_switch = if_else(GENNME != index_med, "switch", NA)) |>
  group_by(ENROLID) |> 
...
oac_users <- all_drug_index_continuous |> 
  ...
  ungroup() |> 
  select(ENROLID, GENNME, age_at_index, SEX, index_date, obj_period_end) |> 
  distinct(ENROLID, .keep_all = TRUE) #57065 
...
analytic_cohort_oac <- left_join(test, ccae_bleed_outcome_no_trauma, by = "ENROLID") |> 
...
precipitant_cohort_6 <- precipitant_cohort_5 %>%
  ...
  group_by(ENROLID, expo_start_date, precipitant) %>%
  mutate(absolute_daysupp = abs(DAYSUPP)) %>%
  filter(
    !(DAYSUPP < 0 & absolute_daysupp %in% DAYSUPP[DAYSUPP > 0]) &
    !(DAYSUPP > 0 & absolute_daysupp %in% abs(DAYSUPP[DAYSUPP < 0]))
